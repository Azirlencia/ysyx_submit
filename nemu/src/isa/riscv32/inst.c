...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 0100011", sw, S, Mw(src1 + imm, 4, src2));//store world
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra , R, R(rd) = (SEXT(BITS(src1, 31, 31), 1) << (32 - src2)) | (src1 >> src2));
  INSTPAT("0000000 ????? ????? 010 ????? 0110011", slt , R, R(rd) = (int32_t)src1 < (int32_t)src2 ? 1:0);
  INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh , R, int32_t a = src1; int32_t b = src2; int64_t tmp = (int64_t)a * (int64_t)b; R(rd) = BITS(tmp, 63, 32));
  INSTPAT("0000000 ????? ????? 011 ????? 0110011", sltu    , R, R(rd) = (uint32_t)src1 < (uint32_t)src2 ? 1:0);
  INSTPAT("???????????? ????? 000 ????? 0010011", li   , I, R(rd) = src1 + imm);//load to reg0
  INSTPAT("???????????? ????? 000 ????? 0010011", mv, I, R(rd) = src1 + imm);
  ...
  INSTPAT("???????????? ????? 000 ????? 1100111", jalr, I, R(rd) = s -> pc + 4; s -> dnpc = (src1 + imm) & ~1);//return(jalr)
  //INSTPAT("??????? ????? 00000 000 ????? 0010011", mv, I, R(rd) = src1);//move
  //INSTPAT("??????? ????? ????? ??? ????? 11011 11", j, J, s->dnpc = s->pc + imm);//jump
  //INSTPAT("??????? ?????????? ?????????? 000 01101 11", j, J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai   , I, imm = BITS(imm, 4, 0); R(rd) = (SEXT(BITS(src1, 31, 31), 1) << (32 - imm)) | (src1 >> imm));
  INSTPAT("0000000 ????? ????? 101 ????? 0010011", srli  ,  I, R(rd)= src1 >> imm);
  INSTPAT("0000000 ????? ????? 001 ????? 0010011", slli  ,  I, R(rd)= src1 << imm);
  INSTPAT("??????? ????? ????? ??? ????? 1101111", jal , J, R(rd) = s -> pc + 4; s -> dnpc += imm -4; );//printf("Executing JAL, rd: %d, PC + 4: %u\n", rd, (unsigned int)(s->pc + 4)); 
  INSTPAT("??????? ????? ????? 001 ????? 1100011", bne , B, s -> dnpc += src1 != src2 ? imm - 4: 0;);
  INSTPAT("??????? ????? ????? 000 ????? 1100011", beq , B, s -> dnpc += src1 == src2 ? imm - 4: 0;);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge , B, s -> dnpc += (int)src1 >= (int)src2 ? imm - 4: 0);
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt , B, s -> dnpc += (int)src1 < (int)src2 ? imm - 4: 0);
  INSTPAT("???????????? ????? 110 ????? 1100011", bltu , B, s -> dnpc += (uint32_t)src1 < (uint32_t)src2 ? imm - 4: 0);
